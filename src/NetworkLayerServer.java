...
public class NetworkLayerServer {
    ...
    static ArrayList<IPAddress> activeClientList;
    ...
    public static void main(String[] args) {
        ...
        applyDVR(1); //Update routing table using distance vector routing until convergence
...
//        stateChanger = new RouterStateChanger();//Starts a new thread which turns on/off routers randomly depending on parameter Constants.LAMBDA
        ...
        while(true) {
            try {
                ...
                System.out.println(activeClientList.size());
...
//            router.printRoutingTable();
    ...
    public static synchronized void applyDVR(int startingRouterId)
    {
        simpleDVR(startingRouterId);
//        DVR(startingRouterId);
    }
        ...
        */
        ...
        while (convergence)
        {
            ...
            for (int i = 0; i < routers.size(); i++) {
                int index = (startingRouterId - 1) % routers.size();
                Router r = routers.get(index);
                System.out.println("Cur router: " + r.getRouterId());
                ArrayList<Integer> neighborList = r.getNeighborRouterIDs();
                for (Integer n : neighborList) {
//                    System.out.println("N ID: "+n);
                    Router neighbor = getRouterFromID(n);
                    if (neighbor.getState()) convergence = neighbor.sfupdateRoutingTable(r) | convergence;
                }
                startingRouterId++;
            }
...
//                    System.out.println("N ID: "+n);
                        Router neighbor = getRouterFromID(n);
                        if (neighbor.getState()) convergence = neighbor.updateRoutingTable(r) | convergence;
                    }
                startingRouterId++;
            }
            System.out.println(convergence);
